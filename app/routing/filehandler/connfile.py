# -*- coding: utf-8 -*-
"""
Created on Mon Jul 22 19:55:08 2019

@author: Dan

connfile.py

import a manual connection file
"""
...
def jsondatafromfile(filename):
    ...
    # Convert from json to connections format
    connections = {}
    for entry in data['connections']:
        connections[tuple(entry["source"])] = [tuple(idx) for idx in entry["targets"]]
    return connections


def datatojson(conn_data, filename):
    """
    Writes json-formated dict to .json file
    :param conn_data: connections format
    :param filename: string output filename
    :return: None, generates file
    """
    # Convert to json format
    out_data = {"connections": []}
    for key in conn_data:
        entry = {"source": key, "targets": conn_data[key]}
        out_data["connections"].append(entry)
...
def format_connections(data):
    """
    Formats a 2D list of connections data to the proper format
    Proper format is Dict
    Keys: numrings (index 0 of list)
    Values: Connected numrings (indices 1: of list)
    :param data: 2D list
